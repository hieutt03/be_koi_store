...
export const updatePackage = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const currentPackage = await PackageService.getPackageByPackageId(Number(packageId));
        if (!currentPackage) {
        ...
        const isSuccess = await PackageService.update(Number(packageId), updateData);
        if (!isSuccess) {
